...
- **Input:** a five digit United States ZIP code, for example `16802`.
...
- **Failure:** `KeyNotConfiguredError` when no key is set,
  `LocationNotFoundError` when the provider has no usable match for that ZIP,
  and `LocationServiceError` when the request itself fails. No message carries
  the key, the request URL, or raw provider exception text, because the key
...
def look_up_zip(postcode: str) -> ZipLocation:
    ...
    return _first_usable_match(payload, postcode)
